...
void draw() {
	...
	switch (gameState) {
		...
		for(int i = 0; i < cabbageX.length; i++){
			...
			if(playerHealth < PLAYER_MAX_HEALTH
			  && isHit(playerX, playerY, cabbageX[i], cabbageY[i])) {
    ...
    // --- Requirement #3: Use boolean isHit(...) to detect clock <-> player collision
      if(isHit(playerX, playerY, clockX[i], clockY[i])){
  
        gameTimer += CLOCK_BONUS_SECONDS;
		...
		for(int i = 0; i < soldierX.length; i++){
			...
			// Requirement #3: Use boolean isHit(...) to detect collision
			if(isHit(playerX, playerY, soldierX[i], soldierY[i])) { 
...
boolean isHit(float ax, float ay, float bx, float by){  // Requirement #3 
  
  if(bx + SOIL_SIZE > ax    // r1 right edge past r2 left
     && bx < ax + SOIL_SIZE    // r1 left edge past r2 right
     && by + SOIL_SIZE > ay    // r1 top edge past r2 bottom
     && by < ay + SOIL_SIZE) { // r1 bottom edge past r2 top
	     return true;
     }else{return false;
   }						
}
...
String convertFramesToTimeString(int frames){	// Requirement #4
  ...
  String sSeconds = nf(seconds, 2);
  
  String sTimer = sMinute + ":" + sSeconds;
